...
def battle(func):
    def inner(**kwargs):
        ...
        num = int(input(player_pokemon.get('name')+'''의 선택은?
    1. 공격
    2. 도망
    번호 입력 : '''))
        if num == 1:
            player_dmg = random.randrange(1,10)
            enemy_pokemon_dmg = random.randrange(1,10)
            player_pokemon['hp'] = player_pokemon.get('hp') - player_dmg
            enemy_pokemon['hp'] = player_pokemon.get('hp') - player_dmg

            print(player_pokemon.get('name')+'이(가) 데미지'+ str(player_dmg) + '점을 적중 !')
            if enemy_pokemon['hp'] <= 0 :
                print('적군'+enemy_pokemon.get('name')+'이(가) 기절함')
                print(player_pokemon.get('name')+'이(가) 경험치 1점 획득!')
                player_pokemon['exp'] = player_pokemon.get('exp') + 1
            elif player_pokemon['hp'] <= 0:
                print(player_pokemon.get('name')+'이(가) 기절함')
            else:
                print(enemy_pokemon.get('name')+'이(가) 데미지'+ str(enemy_pokemon_dmg) + '점을 적중 !')
                print(player_pokemon.get('name')+'의 체력' +  str(player_pokemon['hp']))
        if num == 2:
            enemy_pokemon_dmg = random.randrange(1,10)
            h = random.random() * 10
            print(player_pokemon.get('name')+'가 도망을 시도함...')
            if h == 1:
                print('탈출 성공!')
            else:
                print('탈출 실패 !')
                print('적군'+enemy_pokemon.get('name')+'이(가) 데미지'+ str(enemy_pokemon_dmg) + '점을 적중 !')
    return inner
# 이상해
bulbasaur = {
    "name": "이상해",
